...
func dateFormatter(date string) time.Time {
	...
	timeData, _ := time.Parse(layout, date)
...
func SearchParams(w http.ResponseWriter, r *http.Request) {
	...
	dateF := r.URL.Query().Get("date_from")
	var dateFrom time.Time = time.Now().AddDate(-100, 0, 0)

	if len(dateF) > 0 {
		dateFrom = dateFormatter(dateF)
	}

	dateT := r.URL.Query().Get("date_to")
	var dateTo time.Time = time.Now().AddDate(100, 0, 0)

	if len(dateT) > 0 {
		dateTo = dateFormatter(dateT)
	}
	...
	filter["at"] = bson.M{"$gte": dateFrom, "$lte": dateTo}
